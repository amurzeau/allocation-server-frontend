...
@Component({
    ...
    template: `
        ...
        <td>
          <nz-select class="select-use-full-width" nzShowSearch nzPlaceHolder="Select a nature" [ngModel]="data.type" [nzOptions]="applicationTypes"
            (ngModelChange)="data.type = $event; updateRow()">
          </nz-select>
        </td>
        <td>
          <nz-select style="width: 20ch;" nzShowSearch nzPlaceHolder="Select P/T" nzMode="multiple" [ngModel]="data.eotpOpen" (ngModelChange)="data.eotpOpen = $event; updateRow()" [nzOptions]="eotps"></nz-select>
        </td>
        <td>
